...
const formValidation = Yup.object().shape({
    information: Yup.string().required('این فیلد الزامی است'),
});

export const BaseInformations = () => {
    ...
    useEffect(() => {
        if (projectId !== 0) {
            sendRequest(BackendUrls.project + projectId + '/')
                .then((res) => {
                    setInitialValues(makeInitialValues(res.data.information));
                })
                .catch((err) => {
    ...
    const makeInitialValues = (information: string) => {
    ...
    const saveInformation = (v: { [key: string]: string }, reset: any) => {
    ...
    return (
        <Box sx={{border: ' 0.747929px solid #EAD3D3', borderRadius: '6px', background: '#fff'}}>
            ...
            >
                <Typography>اطلاعات پایه کسب‌وکار</Typography>
            </Box>
            <Box
                sx={{
                    display: 'flex',
            ...
            >
                <Formik
                    ...
                    onSubmit={(values, {resetForm, setErrors}) => {
                        saveInformation(values, resetForm);
                    }}
                    enableReinitialize
                >
                    <Form>
                        <Grid container rowSpacing="10px" sx={{maxWidth: '450px'}}>
                            ...
                            >
                                <RichTextEditor name="information"/>
                            </Grid>
